...
# (see qr-lab-wasm/Cargo.toml's `qr-gen` feature doc). The `qr-gen` build is
...
cargo check -p qr-lab-wasm --target wasm32-unknown-unknown
cargo check -p qr-lab-wasm --target wasm32-unknown-unknown --features qr-gen
