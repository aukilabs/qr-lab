...
def test_robust_scanner_decodes_committed_fixture() -> None:
    ...
    result = qr_lab.scan(image, preset="robust_fast", refine=True)
...
def test_temporal_scanner_resets_and_accepts_non_contiguous_input() -> None:
    ...
    scanner = qr_lab.Scanner(temporal=True)
...
def test_invalid_images_and_configuration_raise_python_exceptions() -> None:
    with pytest.raises(ValueError, match="2-D"):
        qr_lab.scan(np.zeros((2, 3, 4), dtype=np.uint8))
    with pytest.raises(TypeError, match="uint8"):
        qr_lab.scan(np.zeros((3, 4), dtype=np.float32))
    with pytest.raises(ValueError, match="unknown preset"):
        qr_lab.scan(np.zeros((3, 4), dtype=np.uint8), preset="unknown")
    with pytest.raises(RuntimeError, match="configuration"):
        qr_lab.van_cittert(np.zeros((3, 4), dtype=np.uint8), 0.0, 4)
