# QRKit architecture decisions

Status: accepted for the initial workspace implementation (2026-07-10).

## Compatibility and publication

- New packages use the `qrkit-*` names and remain unpublished workspace crates
  for one stabilization cycle.
- `cargo search qrkit` and npm package lookups returned no package collision on
...
- `qrk-core`, its Rust API, current C symbols, WASM exports, and Expo API remain
...
- The Python distribution is `aukilabs-qrkit` and imports as `auki_qrkit`.
  The shorter `qrkit` distribution is an unrelated package on PyPI, so neither
