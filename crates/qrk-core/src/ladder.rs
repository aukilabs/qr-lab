...
impl ScanConfig {
    ...
    /// Keep them in [`ROBUST_FULL_BENCHMARK`] for hard stills / offline.
...
fn has_unconsumed_coherent_pair(pool: &[PooledFinder], codes: &[RobustCode]) -> bool {
    ...
    for i in 0..pool.len() {
        ...
        for j in (i + 1)..pool.len() {
            ...
            let mid = [
                (pool[i].x + pool[j].x) / 2.0,
                (pool[i].y + pool[j].y) / 2.0,
            ];
    ...
    if cfg.enable_low_res_upscaling && !exhausted {
        ...
        if detection_starved && run_starved_2x && !exhausted {
            ...
            if cfg.enable_deblur {
                ...
                let ran_roi2 = l.out.variants.iter().any(|v| {
                    matches!(v.kind, VariantKind::UpscaledRoi { factor } if factor >= 2)
                });
