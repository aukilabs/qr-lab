...
pub(crate) fn box_downscale_half(src: &LumaView) -> Option<(Vec<u8>, usize, usize)> {
...
pub(crate) fn bilinear_upscale_2x(src: &LumaView) -> (Vec<u8>, usize, usize) {
    let (w, h) = (src.width(), src.height());
    let (dw, dh) = (w * 2, h * 2);
    let mut out = vec![0u8; dw * dh];
...
pub(crate) fn bilinear_upscale_3x(src: &LumaView) -> (Vec<u8>, usize, usize) {
    let (w, h) = (src.width(), src.height());
    let (dw, dh) = (w * 3, h * 3);
    let mut out = vec![0u8; dw * dh];
...
pub(crate) fn bilinear_upscale_4x(src: &LumaView) -> (Vec<u8>, usize, usize) {
    let (w, h) = (src.width(), src.height());
    let (dw, dh) = (w * 4, h * 4);
    let mut out = vec![0u8; dw * dh];
    for y in 0..h {
...
pub(crate) fn catmull_rom_upscale_2x(src: &LumaView) -> (Vec<u8>, usize, usize) {
    let (w, h) = (src.width(), src.height());
    let (dw, dh) = (w * 2, h * 2);
...
fn morph_rect<const IS_MAX: bool>(src: &[u8], w: usize, h: usize, k: usize, dst: &mut Vec<u8>) {
    dst.clear();
    dst.resize(w * h, 0);
    let ext = ext::<IS_MAX>;
    let r = k / 2;
...
pub(crate) fn area_downscale(src: &LumaView, dw: usize, dh: usize) -> (Vec<u8>, usize, usize) {
    let (w, h) = (src.width(), src.height());
...
/// noise.
pub(crate) fn background_divide(src: &LumaView, se_px: usize) -> (Vec<u8>, usize, usize) {
    let (w, h) = (src.width(), src.height());
...
pub(crate) fn unsharp_mask(src: &LumaView) -> (Vec<u8>, usize, usize) {
    let (w, h) = (src.width(), src.height());
...
/// confidence = 1 − λmin/λmax ∈ [0,1]; ~0 means isotropic (defocus or no
/// blur — directional processing would be a coin flip and must not fire).
/// Sobel gradients, f64 accumulators, O(N) single pass.
pub(crate) fn structure_tensor_blur_direction(src: &LumaView) -> (f64, f64) {
    let (w, h) = (src.width(), src.height());
    if w < 3 || h < 3 {
        return (0.0, 0.0);
    }
...
pub(crate) fn snap_dir(theta: f64) -> (isize, isize) {
...
pub(crate) fn directional_unsharp(
    src: &LumaView,
    theta: f64,
    len: usize,
) -> (Vec<u8>, usize, usize) {
...
pub(crate) fn edge_rise_extent(src: &LumaView, theta: f64) -> Option<f64> {
    let (w, h) = (src.width(), src.height());
    if w < 3 || h < 3 {
        return None;
    }
    ...
    /// `line[a..=b]`.
    fn rise_of(line: &[u8], a: usize, b: usize) -> Option<(f64, i32)> {
        let (va, vb) = (line[a] as i32, line[b] as i32);
        let amp = vb - va;
        if amp.abs() < MIN_RISE_AMPLITUDE {
            return None;
        }
    ...
    fn measure_line(line: &[u8], step_px: f64, rises: &mut Vec<(f64, i32)>) {
        let n = line.len();
        if n < 3 {
            return;
        }
        ...
        for i in 1..n {
            ...
            if dir != 0 && d != dir {
                if let Some((r, a)) = rise_of(line, start, i - 1) {
        ...
        if dir != 0 {
            if let Some((r, a)) = rise_of(line, start, n - 1) {
                rises.push((r * step_px, a));
            }
        }
    }
    ...
    let mut walk = |x0: isize, y0: isize, rises: &mut Vec<(f64, i32)>| {
        ...
        measure_line(&line, step_px, rises);
    };
    match (dx, dy) {
        (1, 0) => {
            for y in 0..h {
                measure_line(src.row(y), step_px, &mut rises);
            }
        }
        (0, 1) => {
            for x in 0..w {
...
pub(crate) fn van_cittert_directional(
    src: &LumaView,
    theta: f64,
    len: usize,
) -> (Vec<u8>, usize, usize) {
