...
//! Usage: cargo run --release -p qrk-core --example scan_debug -- <name> [scale_max_dim]
use std::fs;
...
fn main() {
    ...
    let grid = qrk_core::TileGrid::build(&view);
    let finders = qrk_core::find_finders(&view, &grid);
    ...
    let trips = qrk_core::group_triplets(&view, &grid, &finders);
