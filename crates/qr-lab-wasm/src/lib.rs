//! WebAssembly bindings for `qr-lab-core`, exposing a single `scan_rgba`
...
//! source→working downscale in Rust via `qr_lab::scan`/`scan_traced`,
...
use qr_lab::image::Gray8Image;
use qr_lab::imgproc::blur::estimate_line_direction;
...
pub struct WasmResult {
    ...
    /// dimensions `qr_lab::downscale_luma` computed for `max_dim`.
    pub scan_width: u32,
    pub scan_height: u32,
}
...
/// directly on the full source, like `qr_lab::detect`); the downscale
/// (when one is needed) happens here, in Rust, via `qr_lab::scan` — see
...
/// elapsed time on wasm too — `qr_lab::StageClock` backs them with
...
pub fn scan_rgba(
    ...
    // `scan`'s own doc comment promises: `qr-lab`'s internals never call
...
/// `qr_lab::ScanConfig`, deserialized from the plain object the debug UI
...
/// reads the authoritative Rust values (`qr_lab::ScanConfig::{BASELINE,
...
/// side by `qr_lab::scan_robust_debug`. `luma` crosses to JS as a single
...
/// escalation ladder (`qr_lab::scan_robust`) and return a
...
///
/// `capture: true` switches to `qr_lab::scan_robust_debug` — identical
...
impl WasmImageProcessor {
    ...
    pub fn estimate_line_blur_luma(
        ...
        let length_px = qr_lab::imgproc::blur::estimate_line_length(view, direction.theta_radians);
...
/// [`qr_lab::ScanSession`]: hold one across a camera stream and feed it
...
impl WasmScanSession {
    ...
    /// and 4). See [`qr_lab::SessionConfig`].
    #[wasm_bindgen(constructor)]
    pub fn new(
        config: JsValue,
        rotation_period: u32,
        ...
        .to_core();
        let defaults = qr_lab::SessionConfig::default();
        let session = qr_lab::SessionConfig {
        ...
        Ok(WasmScanSession {
            inner: qr_lab::ScanSession::new(cfg, session),
        })
    }
...
mod tests {
    ...
    fn scan_result_shape_is_serializable() {
        ...
        let luma = qr_lab::luma_from_rgba(&d, 32, 32);
        let view = qr_lab::LumaView::new(&luma, 32, 32, 32).unwrap();
        let opts = qr_lab::ScanOptions {
            max_working_dim: 0,
            refine: false,
        };
        let det = qr_lab::scan(&view, &opts);
    ...
    fn wasm_result_envelope_without_trace_pins_field_names() {
        let luma = flat_luma();
        let view = qr_lab::LumaView::new(&luma, SIDE, SIDE, SIDE).unwrap();
        ...
        let result = WasmResult {
            detections: qr_lab::scan(&view, &opts),
            trace: None,
    ...
    fn wasm_result_envelope_with_trace_pins_field_names() {
        ...
        let mut trace = qr_lab::Trace::new();
        let opts = qr_lab::ScanOptions {
            max_working_dim: 0,
            refine: false,
        };
        let detections = qr_lab::scan_traced(&view, &opts, &mut trace);
        let result = WasmResult {
            detections,
            trace: Some(trace),
            scan_width: SIDE as u32,
    ...
    fn scan_none_and_some_trace_agree_on_a_real_fixture() {
        ...
        // loads a real golden fixture (qr-lab-wasm has no fixture loader of
        // its own — see `tests/common/mod.rs` in qr-lab-core — so read the
        ...
        let view = qr_lab::LumaView::new(&luma, width, height, width).unwrap();

        let opts = qr_lab::ScanOptions {
            max_working_dim: 1280,
            refine: false,
        };
        let a = qr_lab::scan(&view, &opts);
        let mut trace = qr_lab::Trace::new();
        let b = qr_lab::scan_traced(&view, &opts, &mut trace);
    ...
    fn robust_envelope_field_names_are_pinned() {
        ...
        let cfg = qr_lab::ScanConfig::ROBUST_FULL_BENCHMARK;
        let debug = qr_lab::scan_robust_debug(&view, &opts, &cfg);
    ...
    fn robust_config_roundtrip_and_presets_are_camel_case() {
        ...
        assert_eq!(cfg.to_core(), qr_lab::ScanConfig::default());
        ...
        let presets = super::RobustPresets {
            baseline: super::RobustConfig::from_core(qr_lab::ScanConfig::BASELINE),
            robust_fast: super::RobustConfig::from_core(qr_lab::ScanConfig::ROBUST_FAST),
            robust_full_benchmark: super::RobustConfig::from_core(
                qr_lab::ScanConfig::ROBUST_FULL_BENCHMARK,
            ),
        };
        ...
        assert_eq!(
            json.pointer("/robustFast/maxVariantsPerFrame")
                .and_then(|v| v.as_u64()),
            Some(qr_lab::ScanConfig::ROBUST_FAST.max_variants_per_frame as u64)
        );
        let back: super::RobustConfig =
            serde_json::from_value(json.get("robustFullBenchmark").unwrap().clone()).unwrap();
        assert_eq!(back.to_core(), qr_lab::ScanConfig::ROBUST_FULL_BENCHMARK);
    }
}
