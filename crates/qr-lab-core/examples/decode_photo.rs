...
//! Usage: cargo run --release -p qr-lab-core --example decode_photo -- <path.png> [max_dim=1280]
//! (png is a dev-dependency, so run via `--example`.)
//!
//! Plan 5 Task 6: switched from a manual pre-downscale + `detect()` to
//! [`qr_lab_core::scan`] on the FULL-resolution source view. The old manual
...
//! resolution pixels before `qr_lab_core` ever saw them, which silently
...
use qr_lab_core::{scan, ScanOptions};
...
fn main() {
    ...
    let luma: Vec<u8> = match info.color_type {
        ...
        png::ColorType::Rgba => qr_lab_core::luma_from_rgba(&buf[..w * h * 4], w, h),
    ...
    let view = qr_lab_core::LumaView::new(&luma, w, h, w).unwrap();
