...
//! `qr_lab_core::trace::Trace`'s doc for the full contract.
//!
//! Gated behind `#![cfg(feature = "debug-trace")]` so `cargo test -p
//! qr-lab-core` (the default, feature-off build) skips this file entirely
//! rather than failing to compile against `Trace`'s zero-field no-feature
//! shape (see `src/trace.rs`'s doc comment). Run with:
//! `cargo test -p qr-lab-core --features debug-trace --test decode_trace_gate`.

#![cfg(feature = "debug-trace")]

mod common;

use qr_lab_core::{detect_traced, Trace};
...
/// real codes) must carry 4 [`qr_lab_core::trace::DecodedCodeTrace`] entries
