//! Grayscale image storage and zero-copy views for QRKit pipelines.
//!
...
/// A half-open image rectangle `[x, x + width) x [y, y + height)`.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub struct Rect {
    pub x: usize,
    pub y: usize,
...
impl<'a> Gray8View<'a> {
    ...
    /// Create a checked, zero-copy subview. Coordinates are relative to this
    /// view, and the returned view retains the parent row stride.
...
/// Backward-compatible scanner type name.
