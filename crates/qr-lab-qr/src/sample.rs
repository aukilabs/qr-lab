...
pub(crate) type EdgeFit = qr_lab_geometry::Line2;
...
fn fit_line_tls(pts: &[[f64; 2]]) -> EdgeFit {
    qr_lab_geometry::fit_line_tls(pts).expect("scanner line inputs are validated by the caller")
}
...
pub(crate) fn fit_line_tls_weighted(pts: &[[f64; 2]], weights: &[f64]) -> EdgeFit {
    qr_lab_geometry::fit_line_tls_weighted(pts, weights)
...
pub(crate) fn intersect_lines(a: &EdgeFit, b: &EdgeFit) -> Option<[f64; 2]> {
    qr_lab_geometry::intersect_lines(a, b)
}
