...
pub enum VariantKind {
    /// Primary working-view detection pass (no enhancement).
    Baseline,
    ...
    /// (1 = 0.5×, 2 = 0.25×).
    Pyramid {
        /// Number of 2× box halvings from the box working view (1 = 0.5×, …).
        level: u8,
    },
    /// Tile threshold offset in gray levels (±8).
    ThresholdOffset {
        /// Signed gray-level offset applied to every tile threshold.
        offset: i16,
    },
    ...
    UpscaledRoi {
        /// Resample factor: `1` = pristine crop, `2`/`3` = bilinear upscale.
        factor: u8,
    },
    ...
    DirectionalSharpened {
        /// Estimated blur angle in degrees, snapped to {0, 45, 90, 135}.
        theta_deg: i16,
    },
    ...
    VanCittert {
        /// Estimated blur angle in degrees, snapped to {0, 45, 90, 135}.
        theta_deg: i16,
        /// Line-PSF length in pixels at source scale.
        len: u16,
    },
...
pub struct VariantRecord {
    /// Which ladder rung produced this record.
    pub kind: VariantKind,
    /// Ladder stage index (see [`VariantKind::stage`]).
    pub stage: u8,
    /// Per-detection-stage timings within this variant.
    pub timings: StageTimings,
    /// Whole-variant wall time (buffer preparation + detection).
    pub total_ns: u64,
    /// Finder candidates found by this variant.
    pub finders: usize,
    /// Triplets found by this variant.
    pub triplets: usize,
    /// Codes decoded by this variant (before global arbitration).
    pub codes: usize,
...
pub struct RobustCode {
    ...
    /// Ladder rung that first produced this code.
    pub variant: VariantKind,
    /// Ladder stage index of [`Self::variant`].
    pub stage: u8,
}
...
pub struct VariantSnapshot {
    /// Ladder rung whose detection buffer was snapshotted.
    pub kind: VariantKind,
    /// Ladder stage index.
    pub stage: u8,
...
pub struct RobustDebug {
    /// Ladder detections with provenance (same as production output).
