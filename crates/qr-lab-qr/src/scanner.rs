...
pub struct StageTimings {
    /// Tile-threshold construction time in nanoseconds.
    pub tiles_ns: u64,
    /// Finder-pattern detection time in nanoseconds.
    pub finders_ns: u64,
    /// Triplet grouping time in nanoseconds.
    pub triplets_ns: u64,
...
pub struct Detections {
    /// Finder-pattern candidates for this frame.
    pub finders: Vec<FinderCandidate>,
    /// Grouped finder triplets (QR corner hypotheses).
    pub triplets: Vec<TripletCandidate>,
    /// Successfully decoded codes after arbitration.
...
/// `detect_traced`, and `qr-lab-wasm`'s `scan_rgba`) funnels through: runs
...
/// `debug-trace` feature (as `qr-lab-wasm` does, to keep the feature
