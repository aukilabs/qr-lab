...
/// One of the four raster directions used by QR Lab's integer line kernels.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum RasterDirection4 {
    /// Axis-aligned horizontal (+x).
    Horizontal,
    /// Diagonal down-right (+x, +y).
    DownRight,
    /// Axis-aligned vertical (+y).
    Vertical,
...
impl RasterDirection4 {
    /// Integer step `(dx, dy)` for this raster direction.
    ...
    /// Snap a continuous angle to the nearest of the four raster directions.
...
/// Estimated dominant line-blur orientation.
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct DirectionEstimate {
    /// Estimated blur angle in radians.
    pub theta_radians: f64,
    /// Structure-tensor anisotropy in `[0, 1]` (higher = more directional).
    pub confidence: f64,
    /// Angle snapped to a 4-way raster direction for integer kernels.
    pub raster_direction: RasterDirection4,
}

/// Tuning knobs for edge-rise line-length estimation.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct LineLengthConfig {
    /// Minimum number of edge transitions required for a valid estimate.
    pub minimum_transitions: usize,
    /// Minimum edge amplitude (0–255) considered a real transition.
...
/// Like [`estimate_line_length`], with explicit quality thresholds.
