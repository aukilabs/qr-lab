...
use qr_lab_image::{Gray8Image, Gray8View, Gray8ViewMut, Size};
...
/// Configuration for iterative Van Cittert line-PSF restoration.
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct VanCittertConfig {
    /// Blur orientation in radians (snapped to a 4-way raster direction).
    pub theta_radians: f64,
    /// Odd line-PSF support length in pixels.
    pub blur_length: usize,
    /// Number of Van Cittert iterations.
    pub iterations: usize,
    /// Relaxation in `[0, 1]`, quantized to Q16 for deterministic execution.
    pub relaxation: f64,
    /// Border handling for the 1-D line filter.
    pub border: LineBorderMode,
}

impl VanCittertConfig {
    /// Raster direction used by the integer line kernel.
...
/// Configuration for one-shot directional unsharp masking along a line PSF.
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct DirectionalUnsharpConfig {
    /// Blur orientation in radians.
    pub theta_radians: f64,
    /// Odd line-PSF support length in pixels.
    pub blur_length: usize,
...
/// Metadata returned by restoration operators.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct RestorationMetadata {
    /// Raster direction actually used by the kernel.
...
/// Allocate an output image and run [`van_cittert_line_into`].
...
/// Allocate an output image and run [`directional_unsharp_line_into`].
...
/// Apply directional unsharp masking into caller-owned storage.
